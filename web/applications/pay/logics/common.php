...
class Common {
     ...
     * @author yuanxiaolin@dachuwang.com
     */
    public static function DoApi ($url = '', $post_data = array(), $type = "GET") {
        $response = array ();
        if (empty($url)) {
            throw new Exception('url required,but empty be given');
        } else {
            $full_respose = Http::request($url, $post_data, $type);
            $response = json_decode($full_respose, TRUE);
            if ($full_respose === null) {
                $log = self::LogInit();
                $log::ERROR(sprintf('api call error|api_url:%s|api_back:%s', $url, $response));
            }
        }
