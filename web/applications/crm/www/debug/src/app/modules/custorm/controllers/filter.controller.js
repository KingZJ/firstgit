...
angular.module('dachuwang')
  .controller('FilterController', ['$scope','$modalInstance','$window','rpc', 'daChuDialog', 'daChuLocal', function($scope, $modalInstance, $window, rpc, dialog, daChuLocal) {
    ...
    $scope.property = {
      dimensions : [],
      customer_type : [{name:'从未下单',value:0},{name:'有下单记录',value:1}]
    };
    $scope.sift = {};
    ...
    (function() {
      rpc.load('customer/create_input','GET').then(function(msg) {
        if(parseInt(msg.status) === 0) {
          ...
          if(sift && sift.dimensions && sift.dimensions.value){
          ...
          if(sift && sift.shop_type && sift.shop_type.id){
    ...
    $scope.func = {
        ...
        if(isEmptyObject(obj.sift) && isEmptyObject(obj.order.by) && isEmptyObject(obj.order.way)) {
          this.cancel();
          return false;
        }
        // 记录本次筛选条件
      ...
      reset : function() {
        $scope.sift.line = '';
        $scope.sift.dimensions = '';
        $scope.sift.shop_type = '';
        daChuLocal.remove('filter_sift');
      }
    };
  }]);
