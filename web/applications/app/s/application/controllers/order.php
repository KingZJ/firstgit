...
class Order extends MY_Controller {
    ...
    private function _create_order_and_detail($order_info) {
        ...
        //TODO
        $customer_type      = !empty($order_info['customer_type']) ? $order_info['customer_type'] : C('customer.type.normal.value');
        //母订单部分
        $main_order = array(
            'username'           => $user['name'],
            'user_id'            => $user['id'],
    ...
    public function add() {
        ...
        if ($user['invite_id'] == C('customer.public_sea_code')) {
            $invite_bd = $this->MUser->get_one('*', ['id' => $user['invite_bd'], 'role_id' => C('user.saleuser.BD.type'), 'status !=' => C('status.common.del'), 'site_id' => $user['site_id'], 'province_id' => $user['province_id']]);
            if($invite_bd){
                // BD 在职
                $this->MCustomer->update_info(['status' => C('customer.status.undone.code'), 'invite_id' => $user['invite_bd']], ['id' => $user['id']]);
                // 更新订单的所属销售
                $this->MOrder->update_info(['sale_id' => $user['invite_bd'], 'sale_role' => C('user.saleuser.BD.type')], ['in' => ['id' => $order_id_arr]]);
            }else{
                // BD 不在职
                $this->MCustomer->update_info(array('status' => C('customer.status.no_bd.code')), array('id' => $user['id']));
            }
        }
