...
class Shared_customer extends MY_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->model(['MUser', 'MCustomer', 'MCustomer_potential', 'MLine']);
    }
    private function _assemble_res($status, $msg, $res, $total=NULL) {
        $arr = array(
            'status' => $status,
            'msg'    => $msg,
            'res'    => $res
    ...
    public function potential_lists() {
        ...
        $this->_return_json($this->_assemble_res(C('status.req.success'), '请求成功', $lists, $total));
    }
