...
class Complaint extends MY_Controller {
    ...
    public function view() {
        ...
        // 数据查询
        $data = $this->MComplaint->get_one('*', array('id' => $this->input->post('id', TRUE)));
        // 返回结果
        $this->_return_json(
            array(
    ...
    public function create() {
        ...
        $data['deliver_date'] = $order['deliver_date'];
        $data['deliver_time'] = $order['deliver_time'];
        $data['invite_id']    = $order['sale_id'];
        ...
        if ($insert_id = $this->MComplaint->create($data)) {
            ...
            foreach ($contents as $content) {
                ...
                $data['sum_price']    = $product['single_price'] * $content['quantity'] * 100;
    ...
    public function edit_input() {
        ...
        // 数据查询
        $data = $this->MComplaint->get_one('*', array('id' => $_POST['id']));
    ...
    public function edit() {
        ...
        // 删除图片
        $this->MImage->false_delete(['owner_type' => C('complaint.owner_type'), 'owner_id' => $_POST['id']]);
        // 添加图片
        if(!empty($_POST['imgUploads'])) {
            $imgUploads = $_POST['imgUploads'];
            $img_arr = [];
        ...
        // 删除投诉内容
        $this->MComplaint_content->false_delete(['cid' => $_POST['id']]);
        // 添加投诉内容
        $contents = $_POST['contents'];
        $content_arr = [];
        foreach ($contents as $content) {
    ...
    public function delete() {
        ...
        // 删除投诉内容
        $this->MComplaint_content->false_delete(['cid' => $del_id]);
        // 删除投诉对应图片
        $this->MImage->false_delete(['owner_id' => $del_id, 'owner_type' => C('complaint.owner_type')]);
    ...
    private function _format_list($list) {
        ...
        $user_list = $this->MUser->get_lists('id, name', array('in' => array('id' => $user_ids)));
        $user_dict = array_combine(array_column($user_list, 'id'), array_column($user_list, 'name'));

        $cids = array_column($list, 'id');
        $content_list = $this->MComplaint_content->get_lists('*', ['in' => ['cid' => $cids], 'status' => C('status.common.success')]);
        $content_dict = [];
        ...
        $image_list = $this->MImage->get_lists('*', ['owner_type' => C('complaint.owner_type'), 'in' => ['owner_id' => $cids], 'status' => C('status.common.success')]);
        $image_dict = [];
