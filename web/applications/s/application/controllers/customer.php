...
class Customer extends MY_Controller {
    ...
    public function edit_input() {
        ...
        if($data['account_type'] == C('customer.account_type.parent.value')) {
            $mobile = $data['mobile'];
            $where = 'parent_mobile = ' .  $mobile;
            $child_count =  $this->MCustomer->count(array('parent_mobile'=>$mobile));
        ...
        $check_dates= array_merge(array('none'=>array(array('name'=>'无','value'=> 'none'))), $ka_dates);
        // 返回结果
        $this->_return_json(
            array(
                'status'         => C('status.req.success'),
