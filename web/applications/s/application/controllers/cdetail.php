...
class Cdetail extends MY_Controller {
    ...
    private $_customer_info_lists = ['created_time','shop_name','name','mobile','shop_type','dimensions','site_id','address','line_id','remark','lng', 'lat', 'id', 'status','account_type','customer_type'];
    ...
    private function _customer_info() {
        ...
        try {
            ...
            $info['ka_info'] = $info['account_type'] ? $this->_get_ka_info($info['account_type'], $uid) : null;
            unset($info['account_type']);
             */
        } catch (Exception $e) {
    ...
    private function _customer_orders() {
        ...
        $page = $this->get_page();
        $orders = $this->MOrder->get_lists(['created_time','order_number','status'], $where, ['created_time'=>'desc'],NULL, $page['offset'], $page['page_size']);
    ...
    private function _order_detail() {
        ...
        $fields = ['id','sale_id','order_number','total_price','deal_price','created_time','deliver_time','final_price','status'];
        $where = ['order_number' => $order_number];
        try {
            //查订单的主要信息
            $order_info = $this->MOrder->get_one($fields, $where);
            ...
            $order_info['sale_name'] = $sale_name;
