...
.controller('confirmController', ["$rootScope", "$scope", "$filter", "$cookieStore", "$window", "cartlist", "deliver", "req", "rpc", "daChuDialog", 'userAuthService', 'daChuConfig' , 'coupon', 'daChuLocal' , function ($rootScope , $scope, $filter, $cookieStore, $window, cartlist, deliver, req, rpc, daChuDialog, userAuthService, daChuConfig, coupon, daChuLocal) {
  ...
  // cartlist.price = cartlist.items[0].sum;
  // cartlist.price = parseInt(cartlist.price, 10);
  ...
  rpc.load('order/confirm_options', 'POST', {cartlist: cartlist.getDetail()}).then(function(data) {
    $scope.minus_info = data.minus_info.length == 0 ? false : data.minus_info;
  ...
  $scope.payby = function(payway) {
   if(payway == 0) {
      ...
      $scope.pay.weixin = false;
    }else if(payway == 1) {
      $scope.pay.weixin = true;
      $scope.pay.delivery = false;
    }
  ...
  var callBack = function(data) {
    ...
    if(data.status == 0) {
      ...
      if($scope.pay.delivery == true) {
        req.redirect('/order/list/');
      }else if($scope.pay.weixin == true) {
        $rootScope.showLoading = true;
  ...
  $scope.confirm = function(price) {
    ...
    // 判断表单是否填写完整
    // 判断是否选择支付方式
    if($scope.pay.weixin == false && $scope.pay.delivery == false && $scope.showPayType == true) {
      alert('请选择支付方式');
      return;
    }
