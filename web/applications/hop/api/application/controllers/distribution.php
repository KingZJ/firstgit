...
class Distribution extends MY_Controller {
    ...
    public function export() {
        ...
        if(!empty($dist_list)) {
            ...
            // 查询配送单下的订单列表
            $order_by = array('dist_order' => 'ASC', 'created_time' => 'DESC');
            $return_order = $this->format_query('/order/lists', array('dist_ids' => $dist_ids, 'itemsPerPage' => 'all', 'order_by' => $order_by));
            $order_list = $return_order['orderlist'];
            $order_map = array();
            if(!empty($order_list)) {
                foreach ($order_list as $order) {
            ...
            $return_order = $this->format_query('/order/lists', array('dist_ids' => $dist_ids, 'itemsPerPage' => 'all'));
            $order_list = $return_order['orderlist'];
            $order_map = array();
            foreach ($order_list as $order) {
                $order_map[$order['dist_id']][] = $order;
