...
class MY_Controller extends CI_Controller {
    ...
    public function get_page() {
        ...
        $page = empty($_POST['currentPage']) ? 1 : intval($_POST['currentPage']);
        $page_size = empty($_POST['itemsPerPage']) ? 10 : intval($_POST['itemsPerPage']);
    ...
    public function format_query($uri_string, $data = array(), $debug = FALSE) {
        ...
        // 检查是否打开了调试
        if(C('xhprof.switch') && !empty($data) && !empty($data['xhprof'])) {
            $this->s_xhprof = $data['xhprof'];
            unset($data['xhprof']);
        }
        return $data;
    }
