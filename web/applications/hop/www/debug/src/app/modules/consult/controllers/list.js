     ...
     return {
          ...
          initGetTypes:function(){
              req.getdata('/consult/list_options','POST',function(data){
                 ...
                 $scope.statuses=[
                    {msg:data.statuses.processing.msg,code:data.statuses.processing.code},
                    {msg:data.statuses.finish.msg,code:data.statuses.finish.code}
                 ];
              });
          }
     }
  ...
  var common ={
      ...
      exportAll:function(){
        dialog.tips({
          ...
          bodyText: '确定导出全部咨询单吗?',
          ok: function() {
            window.location.href = $scope.site_url+"/consult/export";
          }
        });
      }
  }
