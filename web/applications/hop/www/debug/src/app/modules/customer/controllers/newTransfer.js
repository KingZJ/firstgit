  ...
  var commonL={
      ...
      startTransform:function(){
            if($scope.modelParamL.selTotalCount<=0){ // 是否已选移交客户
                ...
                return;
            }else if($scope.isBeyoundMaxCheck2 ){  // 是否超过客余容量
                dialog.tips({ 
                    actionText: '确定' ,
                    bodyText: '潜在客户客余容量超出了最大限制，无法移交!',
                });
                return;
            }else if($scope.isBeyoundMaxCheck1 ){  // 是否超过客余容量
                dialog.tips({ 
                    actionText: '确定' ,
      ...
      selectDropSaleName:function(){
            ...
            $scope.clickItemL=function(_value,_id,_model) {
               ...
               $scope.$emit("updateModuleL",_model.role_id);
            }
           
            $scope.$watch("modelL.inputModelL",function(_new,_old){
                _new=_new||"";
                if($scope.modelL.changeOther) {return;}
                $scope.modelL.inputId=-1;
            ...
            // $scope.$on("updateModuleL",function(event,_role_id){
            //     $scope.modelParamL.sale_role=$filter("filter")($scope.saleRoleModelL,{role_id:_role_id})[0];
            // });
      },
      //  初始化加载参数列表
      initLoadRequest:function(){
            req.getdata('/customer/list_transfer_options', 'POST', function(data) {
                    if(data.status == 0) {
                      ...
                      //$scope.typeModelG = [data.customer_sea.public.potential,data.customer_sea.public.register];
                      //$scope.typeModelS = $scope.typeModelSC = [data.customer_sea.private.old,data.customer_sea.private.potential,data.customer_sea.private.register];
      ...
      init:function(){
              ...
              // $scope.$watch('modelParamL.selTotalCount',function(param1,param2){
              //       $scope.tipBackground={"background":"#777777"};

              //       if($scope.modelParamL._type_=="g"){
              //           $scope.isBeyoundMaxCheck=false;

              //       }else if($scope.modelParamL._type_=="s"){
              //           if( parseInt($scope.modelParamL.selTotalCount) > parseInt($scope.modelS.selClickModel.max_customer)){
              //              $scope.tipBackground={"background":"red"};
              //              $scope.isBeyoundMaxCheck=true;
              //           } else{
              //              $scope.isBeyoundMaxCheck=false;
              //           }
              //       }
