...
class Promotion extends MY_Controller {
    ...
    public function save() {
        if(isset($this->post['id'])) {
            $data = $this->format_query('/promotion/save', $this->post);
        } else {
            ...
            $this->post['order_type'] = intval($this->post['orderType']) > 0 ? intval($this->post['orderType']) : 0;
    ...
    public function input_options() {
        ...
        $order_types = array();
        $tmp_order_types = $this->order_split->get_config();
        foreach($tmp_order_types as $item) {
            $order_types[] = array(
                'id'   => $item['code'],
                'name' => $item['msg'],
            );
